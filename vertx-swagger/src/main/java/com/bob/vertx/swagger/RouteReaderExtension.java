 ...
 */
public class RouteReaderExtension implements ReaderExtension {

    private static final Logger LOGGER = LoggerFactory.getLogger(RouteReaderExtension.class);
    ...
    private static List<String> parseStringValues(String str) {
        return parseAnnotationValues(str, v -> v);
    }
                ...
                if (!ReflectionUtils.isVoid(cls)) {
                    ...
                    if (property != null) {
                        final Property responseProperty = ContainerWrapper.wrapContainer(header.responseContainer(),
                                property, ContainerWrapper.ARRAY, ContainerWrapper.LIST, ContainerWrapper.SET, ContainerWrapper.MAP);
    ...
    public String getHttpMethod(ReaderContext context, Method method) {
        if (method.isAnnotationPresent(BBRouter.class)) {
            BBRouter bbRouter = method.getAnnotation(BBRouter.class);
            return bbRouter.httpMethod().toString().toLowerCase();
        } else {
            final ApiOperation apiOperation = ReflectionUtils.getAnnotation(method, ApiOperation.class);
    ...
    public void applyTags(ReaderContext context, Operation operation, Method method) {
        ...
        final Class<?> clasz = context.getCls();
        final Api apiAnnotation = clasz.getAnnotation(Api.class);
        if (apiAnnotation != null) {
        ...
        if (tags.size() > 0) {
            ...
            operation.tag(clasz.getSimpleName());
        }
    }
